...
public class PauseMenu extends Menu {
	...
	public void paintComponent(Graphics g) {
		...
		g.drawString(""+win.getMap().getPlayer().getLooker().getLeft(), getWidth()/2-45, 100);
		g.setColor(win.getMap().getPlayer().getColor());
		g.drawString(""+win.getMap().getPlayer().getSymbol(), getWidth()/2-15, 100);
		g.setColor(win.getMap().getPlayer().getLooker().getRightColor());
		g.drawString(""+win.getMap().getPlayer().getLooker().getRight(), getWidth()/2+15, 100);
